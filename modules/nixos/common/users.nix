# modules/nixos/common/users.nix
{ config, lib, pkgs, ... }:
{
  # Your primary user
  users.users.xeseuses = {
    isNormalUser = true;
    description = "Xeseuses";
    extraGroups = [
      "wheel"          # sudo access
      "networkmanager" # network management
      "video"          # access to video devices
      "audio"          # access to audio devices
    ];
    # SSH keys (same on all machines)
    openssh.authorizedKeys.keys = [
      "ssh-rsa "
    ];
    # Shell
    shell = pkgs.bash;
  };

  # eridanus security-dashboard monitor key — allows the dashboard service
  # on eridanus to SSH in as root and pull aureport/AIDE data.
  # PermitRootLogin is "prohibit-password" by default, so key-only root
  # login already works once the key is present.
  users.users.root.openssh.authorizedKeys.keys = [
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIDZ5T6V8wL0DZohdnECBTZGC1I1ChuMNsnK5P8p4fsNw root@eridanus"
  ];

  # Sudo without password for wheel group
  security.sudo = {
    enable = true;
    wheelNeedsPassword = false;
  };
  # Mutable users (can change passwords with passwd)
  users.mutableUsers = true;
}

