# hosts/orion/nsd.nix
#
# NSD: authoritative DNS server for two zones:
#   1. lan.      — all internal hostnames (migrated from Unbound local-data)
#   2. xesh.cc.  — split-horizon: internal clients get direct LAN IPs,
#                  bypassing lyra for services on caelum/eridanus
#
# NSD listens only on 127.0.0.1:5353
# Unbound has stub-zones pointing to it for both zones
# Clients never talk to NSD directly
{ lib, ... }:
{
  services.nsd = {
    enable     = true;
    interfaces = [ "127.0.0.1" ];
    port       = 5353;

    # Only Unbound on localhost can query NSD
    ipTransparent = false;

    zones = {

      # ── lan. zone ──────────────────────────────────────────────────────────
      # All .lan hostnames — migrated from Unbound local-data entries
      # Add new hosts here; Unbound local-data in unbound.nix can be removed
      "lan." = {
        data = ''
          $ORIGIN lan.
          $TTL 300

          @   IN SOA  orion.lan. hostmaster.lan. (
                      2026060601  ; serial — increment when you change records
                      3600        ; refresh
                      900         ; retry
                      604800      ; expire
                      300 )       ; minimum TTL

          @           IN NS   orion.lan.

          ; ── Router ──────────────────────────────────────────────────────
          orion       IN A    10.40.10.1

          ; ── Servers (VLAN40) ────────────────────────────────────────────
          caelum      IN A    10.40.40.101
          andromeda   IN A    10.40.40.104
          eridanus    IN A    10.40.40.117
          cache       IN A    10.40.40.117
          horologium  IN A    10.40.40.106
          ha          IN A    10.40.40.115

          ; ── Management (VLAN30) ─────────────────────────────────────────
          unifi-ap    IN A    10.40.30.120

          ; ── IoT (VLAN50) ────────────────────────────────────────────────
          everything-presence  IN A    10.40.50.100
          bed-presence         IN A    10.40.50.101
          aqara-hub            IN A    10.40.50.103
        '';
      };

      # ── Reverse zone for VLAN40 (servers) ────────────────────────────────
      "40.40.10.in-addr.arpa." = {
        data = ''
          $ORIGIN 40.40.10.in-addr.arpa.
          $TTL 300

          @   IN SOA  orion.lan. hostmaster.lan. (
                      2026060601 3600 900 604800 300 )

          @   IN NS   orion.lan.

          101 IN PTR  caelum.lan.
          104 IN PTR  andromeda.lan.
          106 IN PTR  horologium.lan.
          115 IN PTR  ha.lan.
          117 IN PTR  eridanus.lan.
        '';
      };

      # ── Reverse zone for VLAN10 (LAN) ────────────────────────────────────
      "10.40.10.in-addr.arpa." = {
        data = ''
          $ORIGIN 10.40.10.in-addr.arpa.
          $TTL 300

          @   IN SOA  orion.lan. hostmaster.lan. (
                      2026060601 3600 900 604800 300 )

          @   IN NS   orion.lan.

          1   IN PTR  orion.lan.
        '';
      };

      # ── xesh.cc split-horizon zone ────────────────────────────────────────
      # Internal clients resolve xesh.cc subdomains to LAN IPs directly.
      # This means: no round-trip to lyra for local services.
      # External clients (via Porkbun/public DNS) are unaffected — they
      # still get lyra's public IP from the real authoritative DNS.
      #
      # Only declare subdomains that actually exist and run on caelum/eridanus.
      # Do NOT add services that are still in progress (nextcloud, paperless etc)
      # — add them here when you deploy them.
      "xesh.cc." = {
        data = ''
          $ORIGIN xesh.cc.
          $TTL 300

          @   IN SOA  orion.xesh.cc. hostmaster.xesh.cc. (
                      2026060601  ; serial
                      3600        ; refresh
                      900         ; retry
                      604800      ; expire
                      300 )       ; minimum

          @           IN NS   orion.xesh.cc.
          orion       IN A    10.40.10.1

          ; ── Services on caelum (10.40.40.101) ───────────────────────────
          immich       IN A   10.40.40.101
          audiobooks   IN A   10.40.40.101
          solibieb     IN A   10.40.40.101
          ha           IN A   10.40.40.115

          ; ── Services on eridanus (10.40.40.117) ─────────────────────────
          ; cache.xesh.cc is not a public subdomain — only cache.lan exists
          ; Add public subdomains here as you deploy them in later phases:
          ; search     IN A   10.40.40.101   ; phase 6 - searx
          ; cal        IN A   10.40.40.101   ; phase 7 - radicale
          ; cloud      IN A   10.40.40.101   ; phase 9 - nextcloud
          ; docs       IN A   10.40.40.101   ; phase 10 - paperless
          ; yt         IN A   10.40.40.101   ; phase 11 - invidious
        '';
      };
    };
  };

  # ── Impermanence ──────────────────────────────────────────────────────────
  environment.persistence."/persist".directories = [
    "/var/lib/nsd"
  ];
}

